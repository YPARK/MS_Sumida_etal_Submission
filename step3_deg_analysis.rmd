...
```{r Fig_gene_scores_subset, fig.width=8, fig.height=3}
...
plot.bulk.sc <- function(.sc, .bulk,
                         .key.genes = c("PRDM1","SGK1","ID3","TRAF3IP3"),
                         fwer.cutoff = .05,
                         tot.cutoff = 500,
                         qv.cutoff = .2,
                         n.top = 7) {
...
.out <- plot.bulk.sc(.sc,
                     .bulk,
                     .key.genes = c("PRDM1","SGK1","LBH","CD6","JUNB","TRAF3IP3"),
                     fwer.cutoff = .01,
                     qv.cutoff = .2,
                     n.top = 5)
```
...
show.each.gene <- function(g, .data, .stat, .cutoff = 2) {
    ...
    lpv.max <- max(-log10(pmax(.temp$pv, 1e-20, na.rm=T)) + 6)
    ...
    p2 <-
        ...
        geom_linerange(linewidth = .2) +
        geom_point(aes(fill = variable, stroke = .2, size = -log10(pmax(pv, 1e-20))), pch = 21) +
        scale_fill_brewer(palette = "Greens", guide="none") +
        scale_size_continuous("p-value", range=c(.5, 2.5), breaks=seq(0, lpv.max, 1), labels = function(x) 10^(-x))
