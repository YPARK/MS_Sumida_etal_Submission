...
```{r Fig_DEG_comparison_mTreg, fig.width=5, fig.height=5, echo = FALSE, results="asis"}
.file <- fig.dir %&% "/Fig_DEG_comparison_mTreg.pdf"
print(.out$plt)
.gg.save(filename = .file, plot = .out$plt, width=5, height=5)
```
...
```{r Fig_DEG_comparison_mTconv, fig.width=5, fig.height=5, echo = FALSE, results="asis"}
.file <- fig.dir %&% "/Fig_DEG_comparison_mTconv.pdf"
print(.out$plt)
.gg.save(filename = .file, plot = .out$plt, width=3, height=5)
```

### Found `r num.int(nrow(unique(hc.ms.deg[fwer < .01, .(gene)])))` unique genes strongly perturbed by MS with FWER 1%

* Up-regulated: `r num.int(nrow(unique(hc.ms.deg[fwer < .01 & z > 0, .(gene)])))`

* Down-regulated:  `r num.int(nrow(unique(hc.ms.deg[fwer < .01 & z < 0, .(gene)])))`
...
```{r Fig_SL_DEG_PRDM1_long, results="asis", echo=F, fig.width=5, fig.height=5}
.file <- fig.dir %&% "/Fig_SL_DEG_PRDM1_long.pdf"
plt <- show.each.gene("PRDM1_long", .hc.ms.dt, hc.ms.deg)
print(plt)
.gg.save(filename = .file, cat.link = T, plot=plt, width=5, height=5)
```


```{r Fig_SL_DEG_PRDM1_short, results="asis", echo=F, fig.width=5, fig.height=5}
