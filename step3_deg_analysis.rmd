...
data.table(gene = .genes.show) %>%
    ...
    mutate(.file=fig.dir %&% "/example/Fig_DEG_example_" %&% gene %&% ".pdf") %>%
    mutate(.link = "[PDF](" %&% .file %&% ") ") %>% 
    select(celltype, hgnc_symbol, z, pv, fwer, .link) %>% 
    mutate(z = num.round(z), pv = num.sci(pv), fwer = num.sci(fwer)) %>% 
    arrange(celltype, pv) %>%
    kbl() %>% 
...
```{r Tab_DEG_OTHER_GENES, results="asis", echo = FALSE}
data.table(gene = additional.genes) %>%
    left_join(hc.ms.deg) %>%
    mutate(is.G=factor(str_starts(`gene`,"ENSG"), c(T,F), c("Gene","Ab"))) %>%
    mutate(.file=fig.dir %&% "/example/Fig_DEG_example_" %&% gene %&% ".pdf") %>%
    mutate(.link = "[PDF](" %&% .file %&% ") ") %>% 
