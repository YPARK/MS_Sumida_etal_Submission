...
```{r Fig_mtconv_sum_membership, fig.width=1.8, fig.height=3}
.sum <-
...
```{r Fig_mtconv_sum_subj_member, fig.width=5, fig.height=3}
.dt <- copy(.sum.subj) %>%
...
```{r Fig_mtreg_sum_membership, fig.width=2, fig.height=3}
.sum <-
    .sum.subj[, .(x = median(x)), by = .(gene, membership)] %>%
...
```{r Fig_mtreg_sum_subj_member, fig.width=6, fig.height=3}
.dt <- copy(.sum.subj) %>%
    mutate(gene = factor(`gene`, .marker.order)) %>%
    mutate(t = subject %&% "." %&% membership) %>%
