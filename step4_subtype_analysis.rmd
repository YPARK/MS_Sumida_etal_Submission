...
bbknn.x <- function(.data, .bbknn, .bbknn.umap) {
    .cols <-
...
```{r Fig_mtconv_sum_membership, fig.width=1.8, fig.height=6}
.sum <-
...
```{r Fig_mtconv_sum_subj_member, fig.width=5, fig.height=6}
.dt <- copy(.sum.subj) %>%
...
```{r Fig_mtreg_sum_membership, fig.width=2, fig.height=6}
.sum <-
    .sum.subj[, .(x = median(x)), by = .(gene, membership)] %>%
    mutate(col = `gene`, row = membership, weight = x) %>%
    col.order(1:10, TRUE) %>%
...
```{r Fig_mtreg_sum_subj_member, fig.width=6, fig.height=6}
.dt <- copy(.sum.subj) %>%
    mutate(gene = factor(`gene`, .marker.order)) %>%
    mutate(t = subject %&% "." %&% membership)
