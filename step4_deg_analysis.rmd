...
.read.stat <- function(data.file,
                       .name,
                       col.file,
                       row.file = "result/step4/rna_final.rows.gz") {
...
# Result: MS vs. HC for high-level T-cells
...
.mkdir("result/step4/temp")
.fwrite(unique(.temp[, .(tag)]), "result/step4/temp/ms_vs_hc.select.gz")
```


```{sh select_annotated_disease_cells}
[ -d result/step4/deg ] || mkdir -p result/step4/deg/
[ -d result/step4/aggregate/ ] || mkdir -p result/step4/aggregate/

if ! [ -f result/step4/deg/ms_vs_hc.mtx.gz ] ; then
	
	[ -f result/step4/temp/ms_vs_hc.mtx.gz ] || \
		mmutil_select_col \
			result/step1/matrix_final.mtx.gz \
			result/step1/matrix_final.cols.gz \
			result/step4/temp/ms_vs_hc.select.gz \
			result/step4/temp/ms_vs_hc \
		2> /dev/null

	mmutil_filter_row \
		--mtx result/step4/temp/ms_vs_hc.mtx.gz \
		--row result/step1/matrix_final.rows.gz \
		--col result/step4/temp/ms_vs_hc.cols.gz \
		--score NNZ \
		--cutoff 100 \
		--col_cutoff 500 \
		--out result/step4/deg/ms_vs_hc \
		2> /dev/null

	rm -f result/step4/temp/ms_vs_hc.*
fi
```

```{r create_cfa_input, include = FALSE}
.cols <- .fread("result/step4/deg/ms_vs_hc.cols.gz", col.names="tag")
.hash <- fread("data/hashtag.tsv.gz", header=TRUE)
...
.fwrite(.input[,.(.tag, celltype)], "result/step4/deg/ms_vs_hc.annot.gz")
.fwrite(.input[,.(batch)], "result/step4/deg/ms_vs_hc.batch.gz")
.fwrite(.input[,.(ind)], "result/step4/deg/ms_vs_hc.ind.gz")
.fwrite(.input[,.(disease)], "result/step4/deg/ms_vs_hc.trt.gz")
.fwrite(distinct(.input[,.(celltype)]), "result/step4/deg/ms_vs_hc.lab.gz")
```

```{sh run_cocoa_ms_vs_hc}
mkdir -p result/step4/deg

[ -f result/step4/deg/bbknn_ms_vs_hc.factors.gz ] || \
	mmutil_bbknn \
		--mtx result/step4/deg/ms_vs_hc.mtx.gz \
		--col result/step4/deg/ms_vs_hc.cols.gz \
		--batch result/step4/deg/ms_vs_hc.batch.gz \
		--knn 100 --rank 10 --log_scale \
		--out result/step4/deg/bbknn_ms_vs_hc \
		2> /dev/null

[ -f result/step4/deg/ms_vs_hc.resid_mu.gz ] || \
	mmutil_cfa_col \
		--mtx result/step4/deg/ms_vs_hc.mtx.gz \
		--col result/step4/deg/ms_vs_hc.cols.gz \
		--annot result/step4/deg/ms_vs_hc.annot.gz \
		--trt result/step4/deg/ms_vs_hc.trt.gz \
		--ind result/step4/deg/ms_vs_hc.ind.gz \
		--lab result/step4/deg/ms_vs_hc.lab.gz \
...
if(!file.exists(.file)) {
    deg.dt <-
        read.deg.result("result/step4/deg/ms_vs_hc") %>%
...
cat("[DEG_MS_vs_HC download]("%&% .file %&% ")\n\n")
```
...
.mkdir("Tab")
.file <- "Tab/MS_vs_HC_pseudobulk_data.txt.gz"
if(!file.exists(.file)) {
    pb.data <- read.deg.result("result/step4/deg/ms_vs_hc")
...
```{r}
.out <- plot.bulk.sc(deg.dt[celltype == "mTreg"],
                     bulk.mTreg.dt, fwer.cutoff = 1e-2, qv.cutoff = .2)
```
...
```{r}
.out <- plot.bulk.sc(deg.dt[celltype == "mTconv"],
                     bulk.mTconv.dt, fwer.cutoff = 1e-2, qv.cutoff = .2)
```
...
for(g in .genes.ms){
    ...
    if(!file.exists(.file)) {
        plt <- show.each.gene(g, pb.data, deg.dt)
